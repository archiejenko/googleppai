...
function RepCard({ rep }: { rep: RepOnboardingData }) {
  ...
  return (
    <div className="bg-[rgb(var(--bg-deep))] border border-[rgb(var(--border-default))] rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-sm text-[rgb(var(--text-primary))] font-mono font-bold">{rep.rep_name}</p>
          <p className="text-xs text-[rgb(var(--text-muted))] mt-0.5">{rep.tenure_months}mo tenure</p>
        </div>
      ...
      {/* Progress bar */}
      <div className="mb-2">
        <div className="flex justify-between text-xs font-mono text-[rgb(var(--text-muted))] mb-1">
          <span>0</span>
          <span>Target: {rep.target_score.toFixed(0)}</span>
        </div>
        <div className="h-3 bg-[rgb(var(--border-default))] relative overflow-hidden">
          <div
        ...
        <div className="flex justify-between text-xs font-mono mt-1">
          ...
          <span className="text-[rgb(var(--text-muted))]">{rep.progress_pct.toFixed(0)}%</span>
        </div>
      </div>
    </div>
  )
...
export default function OnboardingAccelerationPanel({ repId }: Props) {
  ...
  return (
    <div className="bg-[rgb(var(--bg-surface-raised))] border border-[rgb(var(--border-default))] rounded-lg overflow-hidden">
      <div className="px-6 py-4 border-b border-[rgb(var(--border-default))] flex items-center gap-3">
        <Rocket size={18} className="text-[#10B981]" />
        <div>
          <h2 className="font-display font-bold text-[rgb(var(--text-primary))] tracking-wide">
      ...
      {!isLoading && reps.length > 0 && (
        <>
          ...
          {isManager && reps.some(r => r.score_trend.length >= 2) && (
            <div className="px-4 pb-4 border-t border-[rgb(var(--border-default))] pt-4">
              <p className="text-xs text-[rgb(var(--text-muted))] font-mono mb-3 uppercase tracking-wider">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                  ...
                  />
                  <Tooltip
                    contentStyle={{ background: 'rgb(var(--bg-deep))', border: '1px solid rgb(var(--border-default))', borderRadius: 0 }}
